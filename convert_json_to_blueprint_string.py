import sys, os, json, zlib, base64, pyperclip
...
def writeJsonFile(filename,absolute_path = False):
	if absolute_path:
		with open(os.path.join(filename)) as f:
				with open(filename+".json","w+") as g:
					g.write(json.dumps(convertoToJson(f.read()),indent=2))
	else:
		with open(os.path.join("blueprints\\"+filename)) as f:
			with open("blueprints\\"+filename+".json","w+") as g:
				g.write(json.dumps(convertoToJson(f.read()),indent=2))

def writeFileFromJson(filename):
	with open("blueprints\\"+filename+".json") as f:
		with open("blueprints\\"+filename,"w+") as g:
			g.write(convertoToBlueprint(json.loads(f.read())))
# writeJsonFile("C:\\Users\\brade.DESKTOP-E538E75\\OneDrive\\Desktop\\New folder\\_autosave1\\_autosave1\\level.dat0", True)
